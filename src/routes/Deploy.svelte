<script>
	...
	import { push_site, buildSiteBundle } from './Deploy.js'
	import PrimaryButton from '$lib/ui/PrimaryButton.svelte'

	let stage = 'INITIAL'

	let active_deployment = null
	...
	async function build_files() {
		const all_files = await buildSiteBundle({ pages: $page.data.pages })
		files = _.uniqBy(
			all_files.map((file) => {
				console.log({ file })
				return {
	...
	async function deploy_to_repo() {
		...
		await supabase.from('sites').update({ active_deployment }).eq('id', $page.data.site.id)
		stage = 'CONNECT_REPO__ACTIVE__SUCCESS'
		loading = false
	}
	...
	async function get_repos() {
		const res = await Promise.all([
			axios.get(`https://api.github.com/users/${github_account.login}/repos?per_page=100`),
			axios.get(`https://api.github.com/users/${github_account.login}/repos?per_page=100&page=2`)
...
<div class="Deploy primo-reset">
	...
	{#if stage === 'INITIAL'}
		<div class="container">
			<p>
				Primo sites are static sites, which means they can be hosted anywhere. You can download your
				site and host it yourself, or you can deploy it to Github and deploy it automatically to
				most hosting providers.
			</p>
			<p>
				To deploy to Github, you'll need to <a
					href="https://docs.primocms.org/getting-started"
					target="blank"
				>
			...
			<div class="buttons">
				...
				{:else}
					<button disabled class="primo-button primary" on:click={() => (stage = 'CONNECT_GITHUB')}>
						<Icon icon="mdi:github" />
